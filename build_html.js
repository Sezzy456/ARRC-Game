...
<body>
    ...
    const TRASH_TYPES = {
        ...
        paper:   { id: 'paper',   name: 'Blue/Paper', color: 0x2196f3, colorHex: '#2196f3', key: '2', items: ['Box', 'Carton', 'Newspaper'], sprites: ['rubbish_paper_box', 'rubbish_paper_carton', 'rubbish_paper_news'] },
    ...
    class MainScene extends Phaser.Scene {
        ...
        init() {
            this.state = {
                ...
                upgrades: {
                    ...
                    gateAuto: { lvl: 0, max: 5, cost: 1, reqLvl: 3 },
                    ...
                    sanitiser: { lvl: 0, max: 1, cost: 15, reqLvl: 3 },
                    autoSort: { lvl: 0, max: 5, cost: 15, reqLvl: 4 }
                },

                // Bin Upgrades & Workshop Tier Unlocks
                binUpgrades: {
                    organic: { bgUnlocked: false, shopUnlocked: false, tier2Unlocked: false, tier3Unlocked: false, cheaperT1: false, reqLvlBg: 1, reqLvlShop: 1 },
                    paper:   { bgUnlocked: false, shopUnlocked: false, tier2Unlocked: false, tier3Unlocked: false, cheaperT1: false, reqLvlBg: 1, reqLvlShop: 1 },
                    glass:   { bgUnlocked: false, shopUnlocked: false, tier2Unlocked: false, tier3Unlocked: false, cheaperT1: false, reqLvlBg: 1, reqLvlShop: 1 },
                    plastic: { bgUnlocked: false, shopUnlocked: false, tier2Unlocked: false, tier3Unlocked: false, cheaperT1: false, reqLvlBg: 1, reqLvlShop: 1 }
                },
        ...
        create() {
            ...
            this.time.addEvent({
                ...
                callback: () => {
                    ...
                    if (this.state.sanitiserCooldown > 0) {
                        ...
                        if (this.btnSanitiser && this.btnSanitiser.active) {
            ...
            // Keyboard Shortcuts: 1-4 for bins, Space for Tip, C for Gate, V for Sanitise
        ...
        triggerBusRushEvent() {
            ...
            const labelText = this.add.text(50, 16, '🚌 BUS (TAP +10)', {
            ...
            busContainer.on('pointerup', () => {
                ...
                if (this.tweens && this.tweens.add) {
                    ...
                    this.tweens.add({
                        ...
                        alpha: 0,
                        duration: 250,
                        onComplete: () => busContainer.destroy()
                    });
                } else {
                    floatTxt.destroy();
        ...
        generateQuests() {
            ...
            // Progression Quests: MAINTAIN EXACTLY 3 QUESTS AT ALL TIMES
            const tier = this.state.questTier;

            const poolTier0 = [
                { id_base: 't0_org', itemKey: 'organic', name: 'Green Tokens', isResource: true, req: 5, rewardCash: 5, color: '#4caf50' },
                { id_base: 't0_pap', itemKey: 'paper', name: 'Blue Tokens', isResource: true, req: 5, rewardCash: 5, color: '#2196f3' },
            ...
            const poolTier1 = [
                ...
                { id_base: 't1_pap', itemKey: 'recycled_paper', name: 'Rec. Paper', isResource: false, req: 2, rewardCash: 30, color: '#64b5f6' }
            ];
            ...
            const poolTier3 = [
                ...
                { id_base: 't3_mir', itemKey: 'mirror', name: 'Mirror', isResource: false, req: 3, rewardCash: 140, color: '#ba68c8' },
                { id_base: 't3_fil', itemKey: 'filament', name: '3D Filament', isResource: false, req: 4, rewardCash: 180, color: '#ffd54f' }
            ];

            const activePool = (tier === 0) ? poolTier0 : (tier === 1) ? poolTier1 : (tier === 2) ? poolTier2 : poolTier3;

            // Fill missing slots up to EXACTLY 3 quests
            let uniqueIdCounter = Date.now();
            while (this.contracts.length < 3) {
                const template = activePool[Math.floor(Math.random() * activePool.length)];
                const questId = \`\${template.id_base}_\${uniqueIdCounter++}\`;
                this.contracts.push({
                    id: questId,
                    ...
                    rewardTokens: 1, // EVERY NON-TUTORIAL QUEST REWARDS EXACTLY 1 QUEST TOKEN
        ...
        buildLayout() {
            ...
            const questHeader = this.add.text(width / 2, questY - 38, questTitle, { fontSize: '14px', style: 'bold', color: '#ffca28' }).setOrigin(0.5);
            this.mainContainer.add(questHeader);
            this.renderContractsUI(width, questY);

            // 2. WORKSHOPS
            if (!this.state.tutorial.active && this.state.unlockedWorkshopsFacility) {
            ...
            // SANITISER BUTTON: Square-shaped button positioned higher up above head item (queueY - 50)
            if (this.state.unlockedSanitiser && this.trashQueue.length > 0) {
                const cdSecs = Math.ceil(this.state.sanitiserCooldown / 1000);
                const btnLabel = cdSecs > 0 ? ('🧪 SAN\\n(' + cdSecs + 's)') : '🧪\\nSAN';

                this.btnSanitiser = this.add.text(startBinX, queueY - 50, btnLabel, {
        ...
        renderContractsUI(w, y) {
            ...
            this.contracts.forEach((c, idx) => {
                ...
                if (c.isTutorial) {
                    ...
                    if (isDone) {
                        btnStatus.setInteractive({ useHandCursor: true }).on('pointerup', () => {
                            if (c.onComplete) c.onComplete();
                        });
                    }

                    this.mainContainer.add([cardBg, txt, btnStatus]);
                } else {
                    ...
                    if (canFulfill) {
                        btnHandIn.setInteractive({ useHandCursor: true }).on('pointerup', () => {
                            if (c.isResource) this.state.resources[c.itemKey] -= c.req;
                            else this.state.crafted[c.itemKey] -= c.req;

                            this.state.money += c.rewardCash;
                            if (c.rewardTokens) this.state.questTokens += c.rewardTokens;
                            this.state.totalQuestsCompleted++;

                            // Remove fulfilled quest
                            this.contracts = this.contracts.filter(contract => contract.id !== c.id);

                            // Step 5 Tutorial trigger: Turning in ANY quest finishes tutorial!
                            if (this.state.tutorial.active && this.state.tutorial.step === 5) {
                                this.triggerTutorialStepFanfare(5);
                                this.state.tutorial.step = 0;
                                this.state.tutorial.active = false;
                            }
                            ...
                            // Tier 1 -> Tier 2: 10 Quest Tokens
                            // Tier 2 -> Tier 3: 20 Completed Quests
                            if (this.state.questTier === 0 && this.state.questTokens >= 3) {
                                this.state.questTier = 1;
                            } else if (this.state.questTier === 1 && this.state.questTokens >= 10) {
                                this.state.questTier = 2;
                            } else if (this.state.questTier === 2 && this.state.totalQuestsCompleted >= 20) {
        ...
        renderWorkshopsUI(w, h, y) {
            ...
            const shops = [
                ...
                {
                    id: 'paper', name: 'BLUE SHOP', color: '#1565c0',
            ...
            shops.forEach((s, idx) => {
                ...
                if (s.unlocked) {
                    ...
                    s.recipes.forEach((r, rIdx) => {
                        ...
                        const isTierUnlocked = (r.tier === 1) || (r.tier === 2 && s.tier2Unlocked) || (r.tier === 3 && s.tier3Unlocked);

                        if (!isTierUnlocked) {
                            const lockLabel = this.add.text(sx, ry, \`🔒 Tier \${r.tier} Locked\`, { fontSize: '9.5px', color: '#888888', style: 'italic' }).setOrigin(0.5);
                    ...
                    const btnBuy = this.add.text(sx, sy - 2, \`BUY WORKSHOP\n(\$5 + 5 Tokens)\`, {
                        fontSize: '10px', style: 'bold', backgroundColor: '#333', color: '#ffd700', align: 'center', padding: 4
                    }).setOrigin(0.5).setInteractive({ useHandCursor: true });

                    btnBuy.on('pointerup', () => {
                        this.openModal('upgrades', s.id + '_shop');
                    });
                        ...
                        onComplete: () => {
                            ...
                            if (isCorrect) {
                                ...
                                const popOne = this.add.text(targetBin.x, targetBin.y - 15, popText, {
                                    fontSize: '15px', style: 'bold', color: mult > 1 ? '#ffd700' : '#00ff00'
                                }).setOrigin(0.5).setDepth(20);
        ...
        getUpgradeListForCategory(catKey) {
            ...
            if (catKey === 'gate') {
                ...
                upgradeList = [
                    ...
                    { id: 'sanitiser', name: 'Sanitiser Station', desc: 'Unlocks SANITISE button (Doubles/Quads head item, 5s CD)', cost: '15 Tokens', lvl: this.state.upgrades.sanitiser.lvl, max: 1, reqLvl: 3, canAfford: !this.state.unlockedSanitiser && this.state.questTokens >= 15, action: () => { this.state.questTokens -= 15; this.state.unlockedSanitiser = true; this.state.upgrades.sanitiser.lvl = 1; } },
